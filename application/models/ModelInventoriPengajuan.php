...
class ModelInventoriPengajuan extends CI_Model {

	var $table = 'inventori_pengajuan';
		...
		if (isset($_GET['filter'])) {
			...
			if (isset($filter->q)) {
				...
				if ($q != '' || $q != null) {
					...
					$this->db->or_like('no_pengajuan', $q);  // Produces: WHERE name != 'Joe' OR id > 50
					$this->db->or_like('nama_supplier', $q);  // Produces: WHERE name != 'Joe' OR id > 50
				}
			}
		}
	...
	public function insert_image($foreign) {
		$path = base_url() . '/assets/images/pengajuan'; 
		...
		// foreach($base64imagestring as $index => $baseimage) {
			...
			$filePath = 'assets/images/pengajuan/' . $fileName;
		
		   	file_put_contents($filePath, $imageData);
			
			// array_push($returnimage, array(
			// 	'kode' => $foreign,
			// 	'src' => 'assets/images/pengajuan/' . $fileName
			// ));
