...
class Pdf extends FPDF{
   ...
   /* setting zona waktu */
	...
	$pdf->AliasNbPages();
	$pdf->AddPage();
	$pdf->SetMargins(1.2,0.6,1.2,0.5); //set margin kiri, atas, kanan, bawah 1cm
	...
	$pdf->SetFont('Arial','B',12); 
	$pdf->Cell(27.2,0.6,'JADWAL LAB '.$this->app_model->find_lab($lab).' TH AKADEMIK '.$thpel.' '.$this->app_model->find_dosen($nidn),0,1,'C');
	$pdf->Ln(0.1);	
	...
	$qty=0;
	foreach($rs as $r){
		$pdf->Cell(2.0,0.37,$r->jam,'TR',0,'L');
		$hari=2;//hari senin 
			$matakul=$this->app_model->find_jadwal_matakul($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn);
			$mkul=$this->app_model->find_matakul($matakul);
			$kls="".$this->app_model->find_jadwal_kelas($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn)."";
				$trimkelas=str_replace('_','',$kls);
				$ak=substr($trimkelas,0,3);
				$bk=substr($trimkelas,5,6);
				$kls=$ak.$bk;
				
			$pdf->Cell(3.6,0.37,$kls.' '.$this->app_model->find_matakul($matakul),'TR',0,'L');
		$hari=3;//hari senin 
			$matakul=$this->app_model->find_jadwal_matakul($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn);
			$dosen=$this->app_model->find_jadwal_dosen($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn);
		...
		$hari=1;//hari senin 
			...
			$kls="".$this->app_model->find_jadwal_kelas($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn)."";
				...
				$bk=substr($trimkelas,5,6);
				$kls=$ak.$bk;
		$pdf->Cell(3.6,0.37,$kls.' '.$this->app_model->find_matakul($matakul),'T',0,'L');
		$pdf->Ln();
		//baris kedua 
		$pdf->Cell(2.0,0.37,'','BR',0,'L');
		$hari=2;
			$dosen=$this->app_model->find_jadwal_dosen($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn);
			$dosenn=$this->app_model->find_dosen($dosen);
		$pdf->Cell(3.6,0.37,$dosenn,'BR',0,'L');
		$hari=3;
			$dosen=$this->app_model->find_jadwal_dosen($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn);
			$dosenn=$this->app_model->find_dosen($dosen);
		...
		$hari=1;
			$dosen=$this->app_model->find_jadwal_dosen($r->kode,$kelas,$smt,$thpel,$lab,$hari,$nidn);
			$dosenn=$this->app_model->find_dosen($dosen);
		$pdf->Cell(3.6,0.37,$dosenn,'B',0,'L');
		$pdf->Ln();
		$no++;
	}
	$pdf->Ln(0.05);
